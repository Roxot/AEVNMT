...
class SimpleJointModel(BaselineModel):
  ...
  def _diagonal_decoder(self, encoder_outputs, target_length, predicted_source_length, hparams):
  ...
  def _infer_source(self, iterator, hparams):
    ...
    with tf.variable_scope("source_inference_encoder") as scope:
      encoder_outputs = self._positional_encoder(target, target_length, predicted_source_length, hparams)

    with tf.variable_scope("source_inference_decoder"):
      inferred_source = self._diagonal_decoder(encoder_outputs, target_length, predicted_source_length, hparams)
